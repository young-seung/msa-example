...
export default class ReadAccountListQueryHandler implements IQueryHandler<ReadAccountListQuery> {
  constructor(
  ...
  private validation(data: Account, password: string): { access: string; id: string } {
    ...
    return {
      id: account.id,
      access: jwt.sign({ id: account.id, email: account.email, name: account.name }, JWT_SECRET, {
  ...
  async execute(query: ReadAccountListQuery): Promise<{ access: string; id: string }> {
    ...
    const data = await this.repository
      .findOneOrFail({ email: query.email, deletedAt: IsNull() })
      .catch(() => {
